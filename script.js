...
function searchClosestStations() {
    ...
    for (var i = 0; i < locations.length; i++) {
        if (locations[i].city.toLowerCase() == locationInput.value.toLowerCase()) {
            var coord_lat = parseFloat(locations[i].lat);
            var coord_lng = parseFloat(locations[i].lng);
            for (var j = 0; j < stations.length; j++) {
                var station_coords = stations[j].pos.split(" ");
                var station_lat = parseFloat(station_coords[0]);
                var station_lng = parseFloat(station_coords[1]);
...
function connectToAPI() {
    ...
    var fmisid = document.getElementById("station_select").value;
    var data_url = "https://opendata.fmi.fi/wfs?service=WFS&version=2.0.0&request=getFeature&storedquery_id=fmi::observations::weather::daily::simple&fmisid=" + fmisid + "&parameters=tday,tmin,tmax&starttime=" + start_date + "&endtime=" + end_date;
    fetch(data_url)
...
function fetchStations() {
    var regions_url = "https://opendata.fmi.fi/wfs?service=WFS&version=2.0.0&request=getFeature&storedquery_id=fmi::ef::stations";
    fetch(regions_url)
        .then(response => response.text())
        .then(data => {
        parseStations(data);
...
function parseStations(data) {
    ...
    for (var i = 1; i < split_data.length; i++) {
        ...
        if (str.search("Automaattinen sääasema") != -1) //only include automatic weather stations
         {
            var fmisid = str.split('codeSpace="http://xml.fmi.fi/namespace/stationcode/fmisid">').pop().split('</gml:identifier>')[0];
            var name = str.split('codeSpace="http://xml.fmi.fi/namespace/locationcode/name">').pop().split('</gml:name>')[0];
            var region = str.split('codeSpace="http://xml.fmi.fi/namespace/location/region">').pop().split('</gml:name>')[0];
            var pos = str.split('<gml:pos>').pop().split('</gml:pos>')[0];
            var begin = str.split('<gml:beginPosition>').pop().split('</gml:beginPosition>')[0];
