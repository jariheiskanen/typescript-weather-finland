...
function searchClosestStations() {
    ...
    if (select.getAttribute("data-prev-search") != locationInput.value.toLowerCase()) {
        ...
        for (let i = 0; i < locations.length; i++) {
            if (locations[i].city.toLowerCase() == locationInput.value.toLowerCase()) {
                let coord_lat = parseFloat(locations[i].lat);
                let coord_lng = parseFloat(locations[i].lng);
                for (let j = 0; j < stations.length; j++) {
                    let station_coords = stations[j].pos.split(" ");
                    let station_lat = parseFloat(station_coords[0]);
                    let station_lng = parseFloat(station_coords[1]);
                    ...
                    if (distance_to_station < 75) {
                        ...
                        let year = parseInt(document.getElementById("year_select").value);
...
function parseData(data) {
    let split_data = data.split("<wfs:member>");
    weather_data = []; //clear previous data
    let weather_obj = {
        date: ""
    };
    for (let i = 1; i < split_data.length; i++) {
        let str = split_data[i];
        let time = str.split('<BsWfs:Time>').pop().split('</BsWfs:Time>')[0];
        //var pos: string = str.split('<gml:pos>').pop().split('</gml:pos>')[0];
        let parameter = str.split('<BsWfs:ParameterName>').pop().split('</BsWfs:ParameterName>')[0];
        let value = str.split('<BsWfs:ParameterValue>').pop().split('</BsWfs:ParameterValue>')[0];
    ...
    if (weather_data.length != 0) {
        ...
        let canvas = document.getElementById("weather_chart");
...
function fetchStations() {
    let regions_url = "https://opendata.fmi.fi/wfs?service=WFS&version=2.0.0&request=getFeature&storedquery_id=fmi::ef::stations";
    fetch(regions_url)
        .then(response => response.text())
        .then(data => {
        parseStations(data);
...
function parseStations(data) {
    ...
    for (let i = 1; i < split_data.length; i++) {
        ...
        if (str.search("Automaattinen sääasema") != -1) //only include automatic weather stations
         {
            let fmisid = str.split('codeSpace="http://xml.fmi.fi/namespace/stationcode/fmisid">').pop().split('</gml:identifier>')[0];
            let name = str.split('codeSpace="http://xml.fmi.fi/namespace/locationcode/name">').pop().split('</gml:name>')[0];
            let region = str.split('codeSpace="http://xml.fmi.fi/namespace/location/region">').pop().split('</gml:name>')[0];
            let pos = str.split('<gml:pos>').pop().split('</gml:pos>')[0];
            let begin = str.split('<gml:beginPosition>').pop().split('</gml:beginPosition>')[0];
...
function drawTable() {
    //display data in table
    let html = "<table><tr><th>Date</th><th>Avg (°C)</th><th>Min (°C)</th><th>Max (°C)</th></tr>";
...
function drawCanvas() {
    ...
    for (let k = 0; k < chart_numbers.length; k++) {
        ...
        if (document.getElementById('toggle_rain').checked) {
            //rainfall markers
            if (highest_rain <= 100) {
        ...
        if (document.getElementById('toggle_snow').checked) {
            //snow depth markers
            if (highest_snow <= 100) {
...
function drawLines(ctx, chart_range, height_padded, pixels_between_days, highest_snow, highest_rain) {
...
function drawPoints(a, ctx, chart_range, height_padded, pixels_between_days, highest_snow, highest_rain) {
    let canvas = document.getElementById("weather_chart");
    window.setTimeout(() => {
        //draw snow/rain first so temperature gets drawn over it
        if (parseInt(weather_data[a].snow) > 0 && document.getElementById('toggle_snow').checked) {
            let max_snow = 100;
            if (highest_snow > 100) {
                max_snow = 200;
            }
        ...
        if (parseInt(weather_data[a].rrday) > 0 && document.getElementById('toggle_rain').checked) {
            let max_rain = 100;
            if (highest_rain > 100) {
                max_rain = 200;
            }
        ...
        if (a > 0) {
            //tday
            if (document.querySelectorAll('.legend_checkbox')[1].checked) {
            ...
            //tmin
            if (document.querySelectorAll('.legend_checkbox')[2].checked) {
            ...
            //tmax
            if (document.querySelectorAll('.legend_checkbox')[0].checked) {
...
function addBarHover(x, y_start, y_end, a, type) {
...
function drawChartHover(x, y, a, color) {
    let hover_div = document.createElement("div");
